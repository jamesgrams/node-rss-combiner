...
function createFeed(feedConfig, entries) {
  ...
  for (var i = 0; i < entries.length; i++) {
    ...
    if( thisEntry.meta && thisEntry.meta.link ) thisEntry.author = thisEntry.meta.link.split("/").reverse()[0];
    var item = {
...
function combine(feedConfig, callback) {
  ...
  if (err == null) {
    ...
    Q
      ...
      .then(_.flatten)
      .then(function (entries) { return _.sortBy(entries.filter(el=>el), sortEntries) })
